...
import { Sparkles, RefreshCw, ArrowRight, Palette } from 'lucide-react';
import { useModern } from '../context/ModernContext';

export default function StyleQuizModern() {
  const { openConsultation } = useModern();
  const [step, setStep] = useState(0);
  const [answers, setAnswers] = useState({ mood: null, palette: null, purpose: null });
  ...
  const questions = [
    {
      ...
      options: [
        ...
        { id: 'industrial', label: 'Modern Industrial Loft', desc: 'Exposed concrete, dark charcoal accents, smoked glass & metal details.' },
    ...
    },
    {
      ...
      options: [
        ...
        { id: 'monochrome', label: 'Monochromatic Obsidian & Slate', colors: ['#07080A', '#1C212B', '#8E94A2', '#F5F7FA'] },
        { id: 'emerald', label: 'Deep Emerald & Champagne Gold', colors: ['#0A382C', '#1D5C4C', '#D4AF37', '#FAF9F6'] },
  ...
  const generateResult = (finalAnswers) => {
    ...
    setResult({ title, summary, recommendation });
  };
  ...
  return (
    <section id="style-quiz" style={{ padding: '120px 24px', background: 'var(--bg-secondary)' }}>
      <div style={{ maxWidth: '960px', margin: '0 auto' }}>
        ...
        {!result ? (
          ...
          <div className="glass-panel" style={{ padding: '48px', textAlign: 'center', animation: 'fadeIn 0.4s ease' }}>
            ...
            <div style={{ background: 'var(--bg-primary)', padding: '24px', borderRadius: 'var(--radius-sm)', border: '1px solid var(--border-gold)', maxWidth: '680px', margin: '0 auto 36px auto', textAlign: 'left' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'center', gap: '16px', flexWrap: 'wrap' }}>
              <button className="btn-gold" onClick={() => openConsultation({ projectType: result.title })}>
                <span>Schedule Consultation For This Style</span>
                <ArrowRight size={18} />
              </button>

              <button className="btn-outline-gold" onClick={resetQuiz}>
                <RefreshCw size={18} />
